...
router.delete('/deleteTracks', async (req, res) => {
  try {
      const { tracks } = req.body; // Массив треков для удаления

      if (!Array.isArray(tracks) || tracks.length === 0) {
          return res.status(400).json({ message: 'Список треков не предоставлен или пуст' });
      }

      // Удаление треков из базы
      const result = await Track.deleteMany({ track: { $in: tracks } });

      return res.status(200).json({ 
          message: `Удалено треков: ${result.deletedCount}` 
      });
  } catch (error) {
      console.error('Ошибка при удалении треков:', error.message);
      return res.status(500).json({ message: 'Произошла ошибка при удалении треков' });
  }
});
