...
// Маршрут для получения логотипа
router.get('/getLogo', async (req, res) => {
    try {
      const { filialId } = req.query;
      if (!filialId) {
        return res.status(400).json({ message: 'filialId не передан' });
      }
  
      const filial = await Filial.findOne({ filialId });
      if (!filial || !filial.logoPath) {
        return res.status(404).json({ message: 'Логотип не найден' });
      }
  
      res.status(200).json({ logoPath: filial.logoPath });
    } catch (error) {
...
// Настройки хранения файлов для логотипов
...
const uploadLogo = multer({ storage: logoStorage });
  ...
  // Маршрут для загрузки логотипа
  router.post('/uploadLogo', uploadLogo.single('logo'), async (req, res) => {
    try {
      ...
      filial.logoPath = `/uploads/logos/${req.file.filename}`;
      await filial.save();
...
// Маршрут для удаления логотипа
router.delete('/deleteLogo', async (req, res) => {
  try {
    const { filialId } = req.body;
    if (!filialId) {
      return res.status(400).json({ message: 'filialId не передан' });
    ...
    const filePath = path.join(__dirname, '..', filial.logoPath);
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
    }

    // Обновляем запись в базе данных
    filial.logoPath = null;
    await filial.save();

    res.status(200).json({ message: 'Логотип удален' });
  } catch (error) {
